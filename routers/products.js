...
router.get(`/:id`, async (req, res) =>{
    const product = await Product.findById(req.params.id).populate('category');//creating reationships .populate('category')
...
router.get(`/get/count`, async (req, res) =>{
    ...
    if(!productCount){res.status(500).json({succsess: false})}
...
router.get(`/get/featured/:count`, async (req, res) =>{
    ...
    if(!featureProducts){res.status(500).json({succsess: false})}
